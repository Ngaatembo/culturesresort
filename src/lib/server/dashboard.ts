...
export const getDashboardStats = createServerFn({ method: "GET" }).handler(async (): Promise<DashboardStats> => {
  ...
  const [today, pending, preparing, completed, bookings, enquiries, items, unavailable] = await Promise.all([
    db
      .prepare(
    ...
    db.prepare("SELECT COUNT(*) as n FROM orders WHERE status = 'pending'").first<{ n: number }>(),
    db.prepare("SELECT COUNT(*) as n FROM orders WHERE status = 'preparing'").first<{ n: number }>(),
    db
      .prepare("SELECT COUNT(*) as n FROM orders WHERE status = 'completed' AND date(created_at) = date('now')")
      .first<{ n: number }>(),
    db.prepare("SELECT COUNT(*) as n FROM bookings WHERE status = 'pending'").first<{ n: number }>(),
