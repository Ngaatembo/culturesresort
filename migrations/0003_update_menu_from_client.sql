-- Updated Cultures Resort food menu from the latest client-supplied menu pages.
-- Existing photos are preserved. Multi-size/portion prices are stored as options.
...
-- Confirmed single-price updates.
...
UPDATE menu_items SET price_cents = 100, updated_at = datetime('now') WHERE name = 'Sadza / Ugali (Isitshwala)';
UPDATE menu_items SET price_cents = 100, description = 'Plain aromatic rice', updated_at = datetime('now') WHERE name = 'Plain Rice (Wali)';
UPDATE menu_items SET price_cents = 100, updated_at = datetime('now') WHERE name = 'Chapati';
UPDATE menu_items SET price_cents = 300, updated_at = datetime('now') WHERE name = 'Fried Potatoes';
UPDATE menu_items SET price_cents = 300, updated_at = datetime('now') WHERE name = 'Chips';
UPDATE menu_items SET price_cents = 300, description = 'Dried vegetables stewed with peanut butter sauce', updated_at = datetime('now') WHERE name = 'Muriwo Une Dovi';
...
-- Items that were on the old seed but are not present on the supplied menu pages.
...
-- Rename the goat-rib item to the printed-menu wording while preserving its existing photo.
UPDATE menu_items
...
INSERT INTO menu_items (kind, category_slug, category_title, name, description, price_cents, featured, available, sort_order)
SELECT 'food','sides','Accompaniments','Plain Aromatic Rice','',100,0,1,11
...
-- Multi-price items. Clear any previous options so this migration is deterministic.
...
INSERT INTO menu_item_options (menu_item_id,label,price_cents,sort_order)
SELECT id,'Portion',500,1 FROM menu_items WHERE name = 'Zvinyenze';
INSERT INTO menu_item_options (menu_item_id,label,price_cents,sort_order)
SELECT id,'Kapoto',900,2 FROM menu_items WHERE name = 'Zvinyenze';
...
UPDATE menu_items SET price_cents = 0 WHERE id IN (SELECT menu_item_id FROM menu_item_options);
